...
const CashRegisterSection = () => {
  ...
  const handleOpenCashRegister = async () => {
    ...
    try {
      ...
      toast({
        ...
        title: "Erreur",
        description:
          error.response?.data?.message || "Impossible d'ouvrir la caisse",
      });
    }
  };
  ...
  const searchInvoices = async () => {
    try {
      ...
      const response = await api.get("/invoices/search-for-cash", {
        params: {
          search: searchTerm,
        },
      });
  ...
  const handlePrintClientReceipt = async (transactionIds) => {
    try {
      const response = await api.post(
        "/cash-receipts/client-receipt-pdf",
        {
          ...
          <Card className="p-8 max-w-md w-full">
            <div className="text-center space-y-4">
              ...
              <p className="text-gray-600">
                Veuillez ouvrir votre caisse pour commencer les encaissements de
                la journée
              </p>
              <Button
            ...
            <div>
              ...
              <div className="flex items-center gap-4 text-sm text-green-700">
                ...
                {cashRegister.employee && ( // ✅ AJOUTER
                  <>
                    <span className="flex items-center gap-1">
                      ...
                      {cashRegister.employee.first_name}{" "}
                      {cashRegister.employee.last_name}
                      {cashRegister.employee.job_title &&
                        ` (${cashRegister.employee.job_title})`}
                    </span>
                    <span>•</span>
                  </>
                ...
                <span className="flex items-center gap-1">
                  ...
                  Ouvert depuis{" "}
                  {format(new Date(cashRegister.opening_date), "HH:mm", {
                    locale: fr,
                  })}
                </span>
              </div>
        ...
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold">
                {parseFloat(
                  cashRegister.mobile_money_amount || 0
                ).toLocaleString()}
              </p>
              <p className="text-xs text-gray-500">FCFA</p>
            </div>
                      ...
                      <div className="flex justify-between items-center">
                        <div className="flex-1">
                          ...
                          <div className="text-sm text-gray-600">
                            {invoice.meter?.user?.first_name}{" "}
                            {invoice.meter?.user?.last_name}
                          </div>
                          {/* ✅ AJOUTER la période */}
            ...
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex gap-2">
                ...
                <div className="text-sm text-yellow-800">
                  La caisse sera clôturée et vous ne pourrez plus effectuer
                  d'encaissements jusqu'à l'ouverture d'une nouvelle caisse.
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-4 space-y-2">
              ...
              <div className="flex justify-between text-sm">
                ...
                <span className="font-bold text-lg">
                  {parseFloat(
                    cashRegister.expected_amount || 0
                  ).toLocaleString()}{" "}
                  FCFA
                </span>
              </div>
