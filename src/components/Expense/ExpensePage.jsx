  ...
  const handleCategorySubmit = async (formData) => {
    try {
      if (categoryModal.editing) {
        await api.put(`/expenses/categories/${categoryModal.editing.id}`, formData);
        toast({
          title: "Succès",
        ...
        <TableRow key={expense.id}>
          ...
          <TableCell>
            <Badge variant="outline">
              {expense.type === 'facture' ? 'Facture' : 
               expense.type === 'reparation' ? 'Réparation' : 
               expense.type === 'achat' ? 'Achat' : expense.type}
            </Badge>
          </TableCell>
          <TableCell>
          ...
          <TableCell>{parseFloat(expense.amount).toLocaleString()} FCFA</TableCell>
  ...
  {expense.attachments && expense.attachments.length > 0 ? (
    expense.attachments.map(attachment => (
      <Button
        ...
        className="mr-1"
      >
        <FileText className="h-4 w-4" /> {/* Icône de fichier */}
          ...
          <TableCell>
            <div className="flex space-x-2">
              <Button
                variant="outline"
                size="sm"
                onClick={() => setModal({ isOpen: true, editing: expense })}
  ...
  <div className="flex items-center justify-between p-4 border-t">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-600">
              {startIndex + 1}-{Math.min(endIndex, pagination.total)} sur {pagination.total}
            </span>
            <Button
              variant="outline"
              size="sm"
...
const ExpenseForm = ({ isOpen, onClose, editingExpense, categories, onSubmit,employees }) => {
    const { toast } = useToast();
    ...
    useEffect(() => {
      if (editingExpense) {
        setFormData({
          ...
          responsible_ids: editingExpense.responsibles?.map(r => r.id.toString()) || [],
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (!formData.title || !formData.amount || !formData.date || !formData.type) {
          toast({
            title: "Erreur",
        ...
        <DialogContent className="max-w-[800px]">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              ...
              <SelectContent>
                {employees
                  .filter(emp => !formData.responsible_ids.includes(emp.id.toString()))
                  .map(employee => (
                    <SelectItem 
                      key={employee.id} 
...
const ExpenseDetails = ({ expense, onClose }) => {
  ...
  const formatDateTime = (dateString) => {
    try {
      if (!dateString) return '—';
      const date = new Date(dateString);
      return isValid(date) ? format(date, 'dd/MM/yyyy HH:mm', { locale: fr }) : '—';
    } catch (error) {
      return '—';
    }
  };
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <p>{formatDate(expense?.date)}</p>              </div>
              <div>
