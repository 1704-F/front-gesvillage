// Composant pour l'export PDF des enregistrements de distribution
...
// Composant PDF pour les enregistrements de distribution
const DistributionRecordsPDF = ({ distributionRecords, sources }) => {
  ...
  Object.keys(recordsBySource).forEach(sourceId => {
    recordsBySource[sourceId].records.sort((a, b) => 
      new Date(b.distribution_date) - new Date(a.distribution_date)
    );
  });
  ...
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        <Text style={styles.header}>Suivi des Enregistrements de Distribution</Text>
        ...
        <View style={[styles.tableRow, styles.tableHeader]}>
          ...
          <Text style={styles.smallCell}>Zone</Text>
          <Text style={styles.smallCell}>Bénéficiaires</Text>
                ...
                {sourceData.records.map((record) => {
                  ...
                  return (
                    <View key={record.id} style={rowStyle}>
                      ...
                      <Text style={styles.smallCell}>{record.volume_distributed} m³</Text>
                      <Text style={styles.smallCell}>{formatDuration(record.distribution_duration)}</Text>
  ...
  {record.quartiers && record.quartiers.length > 0 
    ...
    : '-'
                      ...
                      <Text style={styles.smallCell}>{record.beneficiaries > 0 ? record.beneficiaries : '-'}</Text>
                      <Text style={styles.smallCell}>
...
const DistributionRecordPDFDownloadButton = ({ distributionRecords, sources, filterStatus, sourceFilter }) => {
  let fileNameParts = ['distribution'];
  ...
  return (
    <PDFDownloadLink 
      document={<DistributionRecordsPDF distributionRecords={distributionRecords} sources={sources} />} 
      fileName={filename}
    >
...
export default DistributionRecordPDFDownloadButton;